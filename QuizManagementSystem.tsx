...
import { authService, quizService } from './src/services/api';
...
const QuizManagementSystem = () => {
  ...
  // Load quizzes when user is authenticated
  useEffect(() => {
    if (currentUser) {
      loadQuizzes();
    }
  ...
  const saveQuiz = async () => {
    ...
    try {
      ...
      await loadQuizzes(); // Reload quizzes
    } catch (error: any) {
    ...
    if (editingQuiz) {
      // Quiz creation handled by API call above
    }
  ...
  const submitQuiz = async () => {
    ...
    try {
      ...
      const answersArray = takingQuiz.questions.map((question) => 
        currentAnswers[question.id] ?? -1
      );
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-12 border-slate-200 focus:border-emerald-400 focus:ring-emerald-400 bg-slate-50 focus:bg-white transition-all"
                  required
                />
              </div>
              <div className="space-y-2">
                      ...
                      <CardContent className="pt-6">
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                          <div className="bg-gradient-to-br from-emerald-100 to-emerald-200 rounded-xl p-4 text-center shadow-md">
                            <div className="text-2xl font-bold text-emerald-700">{quiz.questions.length}</div>
                          ...
                          <div className="bg-gradient-to-br from-teal-100 to-teal-200 rounded-xl p-4 text-center shadow-md">
                            <div className="text-2xl font-bold text-teal-700">{quiz.passingScore}%</div>
                          ...
                          <div className="bg-gradient-to-br from-cyan-100 to-cyan-200 rounded-xl p-4 text-center shadow-md">
                            <div className="text-2xl font-bold text-cyan-700">{stats.totalAttempts}</div>
                            <div className="text-xs text-cyan-600 font-semibold mt-1">Total Attempts</div>
                          </div>
                          <div className="bg-gradient-to-br from-blue-100 to-blue-200 rounded-xl p-4 text-center shadow-md">
                            <div className="text-2xl font-bold text-blue-700">{stats.passedAttempts}</div>
                            <div className="text-xs text-blue-600 font-semibold mt-1">Passed</div>
                          </div>
                          <div className="bg-gradient-to-br from-indigo-100 to-indigo-200 rounded-xl p-4 text-center shadow-md">
                            <div className="text-2xl font-bold text-indigo-700">{stats.totalAttempts > 0 ? stats.averageScore : 0}%</div>
                            <div className="text-xs text-indigo-600 font-semibold mt-1">Average Score</div>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
            ...
            <TabsContent value="results">
              <Card className="shadow-lg border-0">
                <CardHeader className="bg-gradient-to-r from-gray-50 to-white">
                  ...
                  <CardDescription>View all student quiz attempts and performance</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="overflow-x-auto">
                    <div className="min-w-full">
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-1">
                        <div className="bg-gradient-to-r from-purple-50 to-blue-50 px-6 py-4 font-semibold text-sm rounded-lg">
                          <div className="grid grid-cols-5 gap-4">
                            <span className="text-purple-700">Student</span>
                            <span className="text-purple-700">Quiz</span>
                        ...
                        {attempts.map(attempt => {
                          ...
                          return (
                            <div key={attempt.id} className="bg-white border rounded-lg px-6 py-4 shadow-sm hover:shadow-md transition-shadow">
                              <div className="grid grid-cols-5 gap-4 items-center">
                                <span className="text-sm font-medium text-gray-700">{student?.name}</span>
                                <span className="text-sm text-gray-600">{quiz?.title}</span>
                                <span className="text-sm font-bold text-gray-800">{attempt.score}%</span>
                                <Badge variant={attempt.passed ? "default" : "destructive"} className="w-fit">
                                  {attempt.passed ? '✅ Pass' : '❌ Fail'}
                                </Badge>
                                <span className="text-sm text-gray-500">{attempt.attemptedAt.toLocaleDateString()}</span>
                              </div>
                            </div>
                          );
                        })}
                ...
                <CardContent>
                  <div className="grid gap-6">
                    {users.filter(u => u.role === 'student').map(student => {
                      ...
                      const avgScore = studentAttempts.length > 0 ? Math.round(studentAttempts.reduce((sum, a) => sum + a.score, 0) / studentAttempts.length) : 0;
                      return (
                        <Card key={student.id} className="p-6 shadow-md hover:shadow-lg transition-shadow">
                          <div className="flex justify-between items-center">
                            ...
                            <div className="grid grid-cols-3 gap-6 text-center">
                              <div className="bg-blue-50 rounded-lg p-3">
                                <div className="text-xl font-bold text-blue-600">{studentAttempts.length}</div>
                                <div className="text-xs text-blue-600 font-medium">Total Attempts</div>
                              </div>
                              <div className="bg-green-50 rounded-lg p-3">
                                <div className="text-xl font-bold text-green-600">{passedAttempts}</div>
                                <div className="text-xs text-green-600 font-medium">Passed</div>
                              </div>
                              <div className="bg-orange-50 rounded-lg p-3">
                                <div className="text-xl font-bold text-orange-600">{avgScore}%</div>
                                <div className="text-xs text-orange-600 font-medium">Average Score</div>
                              </div>
                            </div>
                          </div>
                        </Card>
                        ...
                        <CardHeader className="bg-gradient-to-r from-emerald-50 via-teal-50 to-cyan-50 border-b border-slate-200">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="flex flex-col space-y-2">
                              {lastAttempt && (
                                <Badge variant={lastAttempt.passed ? "default" : "secondary"} className={`text-center ${lastAttempt.passed ? 'bg-gradient-to-r from-emerald-500 to-teal-500' : 'bg-gradient-to-r from-slate-400 to-slate-500'}`}>
                                  Last: {lastAttempt.score}% ({lastAttempt.passed ? '✅ Pass' : '❌ Fail'})
                                </Badge>
                              )}
                              {userAttempts.length > 0 && (
                                <Badge variant="outline" className="text-center border-emerald-300 text-emerald-700">
                                  🏆 Best: {bestScore}%
                                </Badge>
                              )}
                            </div>
                        ...
                        <CardContent className="pt-6">
                          <div className="flex justify-between items-center">
                            <div className="grid grid-cols-3 gap-4 flex-1">
                              <div className="bg-gradient-to-br from-emerald-100 to-emerald-200 rounded-xl p-4 text-center shadow-md">
                                <div className="text-xl font-bold text-emerald-700">{quiz.questions.length}</div>
                                <div className="text-xs text-emerald-600 font-semibold mt-1">Questions</div>
                              </div>
                              <div className="bg-gradient-to-br from-teal-100 to-teal-200 rounded-xl p-4 text-center shadow-md">
                                <div className="text-xl font-bold text-teal-700">{quiz.passingScore}%</div>
                                <div className="text-xs text-teal-600 font-semibold mt-1">To Pass</div>
                              </div>
                              <div className="bg-gradient-to-br from-cyan-100 to-cyan-200 rounded-xl p-4 text-center shadow-md">
                                <div className="text-xl font-bold text-cyan-700">{userAttempts.length}</div>
                        ...
                        {getUserAttempts(currentUser.id.toString()).map(attempt => (
                          <div key={attempt.id} className="bg-white border rounded-lg px-6 py-4 shadow-sm hover:shadow-md transition-shadow">
                            <div className="grid grid-cols-4 gap-4 items-center">
                              <span className="text-sm font-medium text-gray-700">{attempt.quizTitle}</span>
                              <span className="text-sm font-bold text-gray-800">{attempt.score}%</span>
                              <Badge variant={attempt.passed ? "default" : "destructive"} className="w-fit">
                                {attempt.passed ? '✅ Pass' : '❌ Fail'}
                              </Badge>
